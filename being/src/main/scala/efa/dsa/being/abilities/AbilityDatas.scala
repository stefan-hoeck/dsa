...
import efa.core.{Efa, ToXml, Default, StringId}, Efa._
...
object AbilityDatas extends Util with MapFunctions {
  ...
  implicit lazy val AbilityDatasArbitrary = Arbitrary(
    ^^(mapG[AdvantageData],
       mapG[AdvantageData],
       mapG[FeatData])(AbilityDatas.apply)
    )
  ...
  private def mapG[A:Arbitrary:StringId]: Gen[Map[String,A]] = for {
    i  ← Gen choose (0, 10)
